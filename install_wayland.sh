...
# ── 3. determine weston launcher ──────────────────────────────────────────
WESTON_BIN=$(bash ./weston_check.sh) || die "Could not determine Weston binary"
chmod u+s "$WESTON_BIN"
echo "• Using weston binary: $WESTON_BIN"
...
echo \"[kiosk] $WESTON_BIN -- \$ARGS\" >>\$LOG
...
echo "• Weston will launch via $WESTON_BIN"
